...
const save_test_answer = async (_req, _res) => {

	try {
		const question = await test_question_model.findById(_req.params.test_id);
		if (!question) {
			console.error(chalk.bold(`${getTimestamp()} Status Code : 404 -- Error : Question Not Found -- Service : Save Test Answer`));
			return _res.status(404).json({message: "Question not found"});
		}

		const is_result_exist = await test_question_result_model.findOne({test_question_id: _req.params.test_id, user_id: _req.user._id});
		if (is_result_exist) {
			const try_count = is_result_exist.try_count + 1;
			const result = await test_question_result_model.findByIdAndUpdate(is_result_exist._id, {
				user_answer: _req.body.user_answer,
				is_correct: question.answer === _req.body.user_answer,
				try_count: try_count,
			});
			return _res.status(200).json({
		...
		const result = await test_question_result_model.create({
			test_question_id: _req.params.test_id,
		...
		return _res.status(200).json({
			message: "Test Answer Saved",
			status_code: "200",
			status: "success",
			data: result,
		});
	} catch (error) {
		console.error(chalk.bold(`${getTimestamp()} Status Code : 500 -- Error : ${error} -- Service : Save Test Answer`));
		return _res.status(500).json({message: "Server Error"});
	}
};
